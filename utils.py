...
def read_img_features(feature_store):
    ...
    with open(feature_store, "r") as tsv_in_file:     # Open the tsv file.
        ...
        for item in tqdm(reader):
            ...
            if args.zero_feature:
                features[long_id] = np.zeros((36, args.feature_size), dtype=np.float32)
            else:
...
def get_glove_matrix(index_to_word, vector_dim):
    ...
    for i in range(len(index_to_word)):
        ...
        if word == 'others':
            ...
            weights_matrix[i] = glove['.']
        elif word == 'froward':
            weights_matrix[i] = glove['forward']
        else:
            try:
...
def ndtw_initialize():
    ndtw_criterion = {}
    scan_gts_dir = 'tasks/R2R/data/id_paths.json'
