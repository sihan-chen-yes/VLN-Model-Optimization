...
class Evaluation(object):
    ...
    def __init__(self, split_tag,scans, tok ,instrType='instructions',mapFile=''):
        self.error_margin = 3.0
        self.splits = split_tag
        #REVERIE是什么TODO
        bboxDir = osp.join(file_path,'../../','tasks/REVERIE/data','BBox')
    ...
    def score(self,data, evalType='nav'):
        ...
        score_summary = {
            ...
            'oracle_rate': float(oracle_successes)/float(len(self.scores['oracle_visible'])),
            'spl': np.average(spls)
        }
        if evalType == 'whole':
